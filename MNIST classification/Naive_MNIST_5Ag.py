...
for epoch in range(num_epochs):
    ...
    if cent_comm:    
        ...
        for i, (images, labels) in enumerate(mnist_train_centloader):
            ...
            if len(data_storage_cent) < 1:
                data_storage_cent.append((image, label))
            else:  
                data_storage_cent[counter % 1] = (image, label) # Replace the oldest data point
        ...
        for agent_id in range(num_agents):
            ...
            for batch_idx, (images, labels) in enumerate(train_loader):
                image, label = images[0], labels[0]
                # Storage size of 128 data samples for the data.
                if len(data_storage_dist[batch_idx]) < 1:
                    data_storage_dist[batch_idx].append((image, label))
                else:
                    data_storage_dist[batch_idx][counter % 1] = (image, label) # Replace the oldest data point
